...
`prototype/charabilla.jsx` : prototype React v4 qui sert de spécification vivante. Il implémente : formulaire 3 étapes (prénom + titre + sous-titre optionnel type « à 20 mois », paires de mots, univers), grille par paliers avec pointillés, 3 univers avec ~35 croquis SVG intégrés recolorés automatiquement par univers, **bibliothèque personnelle d'illustrations** (upload des PNG générés, nom de fichier = mot, stockage par univers, priorité de résolution : bibliothèque > croquis SVG > emoji), **flux de génération à la volée avec boucle de validation** (mot absent → prompt verrouillé de l'univers assemblé automatiquement avec traduction FR→EN → image déposée → question « convient-elle ? » → validée = ajoutée à la bibliothèque pour toujours / refusée = régénération), édition des mots (modifier, déplacer, supprimer via modale), comptes multi-dicos simulés, modale de commande (5 formats + 3 encadrements), impression via window.print. À remplacer lors du portage : window.storage (API spécifique aux artifacts Claude) par localStorage en Phase 1 puis Supabase en Phase 2 ; la génération manuelle via ChatGPT par l'appel API gpt-image en Phase 5 ; window.print par le rendu PDF serveur en Phase 3. Le mécanisme cqw + containerType sur l'affiche doit être conservé tel quel (c'est lui qui garantit qu'aucun palier ne déborde). `charabilla-100-mots-prompts.csv` : les 100 mots avec prompts Terre de sienne. `lexique-300-premiers-mots.csv` : lexique étendu. `charabilla-pipeline-technique.md` : détails pipeline illustrations + impression. Dossiers d'images (fichiers nommés `mot.png` en français, ex. `chat.png`, `compote.png`) : « Terre de sienne » en cours de génération par la fondatrice ; « Jardin botanique » et « Nuit céleste » à démarrer après validation de leur test de cohérence.
